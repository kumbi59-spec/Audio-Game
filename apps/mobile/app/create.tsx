...
export default function CreateWorld(): JSX.Element {
  ...
  useEffect(() => {
    if (!step) return;
    void speakOnce(
      `${step.prompt}${step.kind === "freeform" && step.helper ? ` ${step.helper}` : ""}`,
    );
    return () => {
      stopNarration();
    };
  ...
  useEffect(() => {
    ...
    void getWizardSuggestions(step.id, draftSnapshot)
      ...
      .catch(() => {
        /* suggestions are optional — never block the wizard */
      })
      .finally(() => {
  ...
  const speakAnswer = useCallback(async () => {
    ...
    if (step?.kind === "choice") {
      ...
      if (matched) advance(matched);
        ...
        void speakOnce(
          "I didn't catch that. Please pick one of the listed options.",
        );
      return;
    }
  ...
  useVoiceCommands({
    ...
    skip: () => {
      if (step && step.kind === "freeform" && !step.required) advance("");
    },
        ...
        {step.kind === "freeform" ? (
          <>
            <TextInput
              ...
              style={[
                styles.input,
                (step.id === "pitch" || step.id === "startingScenario") &&
                  styles.inputMulti,
              ]}
              value={textInput}
        ...
        <View style={styles.nav}>
          ...
          {step.kind === "freeform" && (
            <Pressable
              accessibilityRole="button"
              accessibilityLabel={
                isLast ? "Finish and start adventure" : "Next step"
              }
...
const styles = StyleSheet.create({
  ...
  primaryZone: { backgroundColor: EQ.surface2, borderColor: EQ.border },
  secondaryZone: { backgroundColor: EQ.accentBg, borderColor: EQ.border2 },
  utilityZone: { backgroundColor: EQ.surface, borderColor: EQ.border },
  ...
  btnPressedTransform: {
    transform: [
      { translateY: MOTION.press.translateY },
      { scale: MOTION.press.scaleIn },
    ],
  },
