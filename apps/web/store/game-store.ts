  ...
  addNarrationEntry: (entry) =>
    set((state) => ({
      session: state.session
        ? {
            ...state.session,
            narrationLog: [...state.session.narrationLog, entry],
          }
        : null,
    })),
  ...
  updateFlags: (flags) =>
    set((state) => ({
      session: state.session
        ? {
            ...state.session,
            globalFlags: { ...state.session.globalFlags, ...flags },
          }
        : null,
    })),
    ...
    set((state) => {
      ...
      const newHp = Math.max(
        ...
        Math.min(
          ...
          state.character.stats.hp + delta
        )
      );
      return {
        character: {
          ...state.character,
          stats: { ...state.character.stats, hp: newHp },
        },
      };
    }),
